...
## Purpose
The human-facing download surface: a single static HyperText Markup Language
(HTML) document with inlined JavaScript and Cascading Style Sheets, embedded
into the binary at compile time via `@embedFile`, and the documented
client-side behavior contract the page implements (per-kind tokens, bearer
token, request/response handling, first-download bootstrap). The capability
owns no HTTP serving, EPUB generation, token codec, store access, or
scheduling — those belong to the `server` and `download` capabilities.
